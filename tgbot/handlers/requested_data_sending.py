...
from tgbot.handlers.debts_check.check_depts import check_table as draingang
...
#     retakes_url = await draingang(db.get_user_ln(message.from_user.id))
#     if not retakes_url:
#         await message.answer(text="<b>Ссылка на ведомости пересдач еще не загружена менеджером!</b>")
#     else:
#         await message.answer(text=retakes_url)
